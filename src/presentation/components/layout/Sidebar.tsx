...
const sections: NavSection[] = [
  ...
  },
  {
    label: "Pods",
    items: [
      { to: "/pods", label: "Meus Pods", icon: Server, end: true },
...
export function Sidebar() {
  return (
    <aside className="hidden md:flex w-60 shrink-0 flex-col border-r border-border bg-sidebar sticky top-16 h-[calc(100vh-4rem)] z-10">
      <div className="px-5 pt-5 pb-2 text-label">GPUaaS</div>
      <nav className="flex-1 overflow-y-auto py-2">
        {sections.map((section, i) => (
          <div key={i} className="mb-4 px-3">
            {section.label ? <div className="text-label px-2 mb-1.5">{section.label}</div> : null}
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        [
                          ...
                          isActive ? "bg-primary-light text-primary" : "text-foreground/80 hover:bg-muted",
                        ].join(" ")
                      }
                    >
                      {({ isActive }) => (
