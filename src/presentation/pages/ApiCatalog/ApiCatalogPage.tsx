...
import type { ModelCategory } from "@/domain/entities";
...
const catIcon: Record<ModelCategory, React.ComponentType<React.SVGProps<SVGSVGElement>>> = {
  "LLM": MessageSquare,
...
const catColor: Record<ModelCategory, string> = {
...
export function ApiCatalogPage() {
  ...
  return (
    <div>
      <PageMeta title="Catálogo de Modelos" description="Modelos prontos para usar via API de inferência." />
          ...
          {categories.map((c) => (
            <button
              ...
              className={`text-xs px-2.5 py-1.5 rounded-full border transition-colors ${
                cat === c ? "bg-primary text-primary-foreground border-primary" : "border-border text-foreground/70 hover:bg-muted"
              }`}
            >
              {c}
            </button>
