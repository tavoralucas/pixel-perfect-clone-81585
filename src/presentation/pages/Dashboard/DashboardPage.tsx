...
import {
  Activity, Server, DollarSign, Wallet, ArrowUpRight, Plus, KeyRound, Boxes,
  CheckCircle2, AlertTriangle, XCircle,
} from "lucide-react";
import { Card, Badge, Btn } from "@/presentation/components/ui/ui-kit";
...
export function DashboardPage() {
  ...
  const usageQ = useQuery({ queryKey: ["usage-series"], queryFn: () => api.getUsageSeries() });
  const activityQ = useQuery({ queryKey: ["recent-activity"], queryFn: () => api.getRecentActivity() });
  ...
  return (
    <div>
      <PageMeta title="Dashboard" description="Resumo geral da sua conta GPUaaS." />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-2">
            <Link
              to="/pods/deploy"
              className="inline-flex items-center justify-center gap-1.5 rounded-md font-medium transition-colors bg-primary text-primary-foreground hover:bg-primary-hover h-9 px-3.5 text-sm w-full"
            >
              <Plus className="h-4 w-4" /> Novo Pod
            </Link>
            <Link
              to="/api/keys"
              className="inline-flex items-center justify-center gap-1.5 rounded-md font-medium transition-colors bg-surface border border-border-strong text-foreground hover:bg-muted h-9 px-3.5 text-sm w-full"
            >
