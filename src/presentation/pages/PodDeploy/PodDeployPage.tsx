...
export function PodDeployPage() {
  const api = useApi();
  const navigate = useNavigate();
  const { data: gpus = [] } = useQuery({ queryKey: ["gpus"], queryFn: () => api.listGpus() });
  ...
  const selectedGpu = useMemo(
    () => gpus.find((g) => g.name === gpu) ?? gpus.find((g) => g.name === "A40") ?? gpus[0],
    [gpus, gpu],
  );
  const selectedTemplate = useMemo(
    () => templates.find((t) => t.name === template) ?? templates[0],
    [templates, template],
  );

  const totals = useMemo(() => {
    const price = selectedGpu?.pricePerHour ?? 0;
    const gpuCost = price * qty;
    const disk = 0.02;
    const vol = 0.035;
    const idle = 0.07;
  ...
  async function handleDeploy() {
    if (!selectedGpu || !selectedTemplate) return;
    await api.deployPod({
  ...
  return (
    <div>
      <PageMeta title="Deploy de Pod" description="Configure e inicie uma nova instância GPU em segundos." />
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div>
          ...
          {step === 2 && (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {templates.slice(0, 9).map((t) => {
                const active = (template || selectedTemplate?.name) === t.name;
                return (
                  <button
                    key={t.name}
            ...
            <div>
              <div className="grid gap-3 sm:grid-cols-3 mb-5">
                {gpus.filter((g) => g.badge).map((g) => {
                  const active = (gpu || selectedGpu?.name) === g.name;
                  return (
                    <button
                      key={g.name}
              ...
              <div className="grid gap-3 sm:grid-cols-2 mt-6">
                {([
                  { id: "ondemand", t: "On-Demand", d: "Pague conforme o uso.", extra: `R$ ${(selectedGpu?.pricePerHour ?? 0).toFixed(2)}/h` },
          ...
          <div className="flex justify-between mt-6">
            ...
            {step < 4 ? (
              ...
              <Btn onClick={handleDeploy}>Deploy Pod</Btn>
            )}
          </div>
        </div>
        ...
        <aside className="lg:sticky lg:top-20 h-fit">
          <Card className="p-5">
            ...
            <Btn className="w-full mt-4" onClick={handleDeploy}>Deploy Pod</Btn>
          </Card>
        </aside>
      </div>
    </div>
