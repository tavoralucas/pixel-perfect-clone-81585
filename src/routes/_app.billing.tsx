...
import { billingDaily } from "@/lib/mockData";
...
export const Route = createFileRoute("/_app/billing")({
  head: () => ({ meta: [{ title: "Billing — GPU Cloud" }] }),
  ...
  return (
    <div>
          ...
          <div className="lg:w-[420px]">
            ...
            <div className="flex flex-wrap gap-2">
              {["R$ 50", "R$ 100", "R$ 200", "R$ 500"].map((v) => (
                <button key={v} className="px-3 py-1.5 text-sm rounded-md border border-border hover:border-primary hover:bg-primary-light">
                  {v}
                </button>
              ))}
      ...
      <Card className="p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Empilhado por tipo de serviço</p>
          </div>
        </div>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={billingDaily}>
              ...
              <Bar dataKey="pods" name="Pods GPU" stackId="a" fill="var(--color-chart-1)" radius={[0, 0, 0, 0]} />
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        {[
          { label: "Pods GPU", value: "R$ 648,30", pct: "72,7%", tone: "primary" as const },
          { label: "API Inference", value: "R$ 127,40", pct: "14,3%", tone: "info" as const },
          { label: "Storage", value: "R$ 116,70", pct: "13,0%", tone: "neutral" as const },
        ].map((c) => (
          <Card key={c.label} className="p-5">
            <div className="flex items-center justify-between">
              <div className="text-xs uppercase tracking-wide text-muted-foreground font-medium">{c.label}</div>
              <Badge tone={c.tone}>{c.pct}</Badge>
            </div>
